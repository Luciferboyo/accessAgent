...
- 不要合并多个操作到一个步骤中
- 【重要】需要打开某个 App 时，必须使用 open_app(包名) 动作，禁止规划"滑动打开应用抽屉"、"搜索应用名"等手动步骤
  - 如果用户提供了包名，直接使用；如果没有提供，第一步规划为"用 open_app 打开 XXX（如不知道包名，先用 search_web 或 home 找到后再 open_app）"
  - 常见包名参考：微信=com.tencent.mm / 飞书Lark=com.ss.android.lark / 抖音=com.ss.android.ugc.aweme / 设置=com.android.settings
