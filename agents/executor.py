...
- back()：返回上一页
- home()：回到主屏幕
- find_package(keyword)：查询设备上安装的包名（不知道应用包名时先用此查询，结果会作为提示返回，再调用 open_app）
- open_app(package)：打开指定包名的应用（必须使用真实包名，不能猜测）
...
- back()：返回上一页
- home()：回到主屏幕
- find_package(keyword)：查询设备上安装的包名（不知道包名时先查询，结果会作为提示返回）
- open_app(package)：打开指定包名的应用（必须使用真实包名）
