...
- 典型情形：
  ...
  ✓ 目标"找到图片消息并发起转发" → tap 缩略图后进入了图片查看器（有 Forward 按钮可用）→ progress（还需点击 Forward）
...
📋 关于 UI 信息的展示形式：
- 为节省 token，UI 变化将以「差异摘要」形式给出：未变化的元素只给数量，新增/消失/变更的元素才显示具体内容
- "新增"行 = 操作后才出现的界面内容（重要：暗示成功进入新页面/弹出新菜单/出现成功标志）
- "消失"行 = 操作后不再可见的内容（重要：暗示原页面被关闭/元素被移除）
- 若摘要显示"完全无变化"，按上方"click/long_click 无变化"规则强制判 stuck
...
# 触发"全量模式"的阈值：UI 行数较少时直接发完整文本更便宜也更准
_FULL_DUMP_LINE_THRESHOLD = 25
# diff 模式下，新增/消失行各自最多保留的条数（防止极端 case 撑爆 prompt）
_DIFF_MAX_LINES_PER_BUCKET = 40


def _ui_to_lines(ui_text: str) -> list[str]:
    """将 parse_elements 输出按行切分并去掉空行。"""
    return [ln for ln in ui_text.splitlines() if ln.strip()]


def _format_ui_diff(ui_before: str, ui_after: str) -> tuple[str, bool]:
    """
    将 before/after 转为紧凑 diff 摘要。
    返回 (text, has_changes)。
    - 小 UI（任一侧行数 <= _FULL_DUMP_LINE_THRESHOLD）：直接返回完整对照（更易判读）
    - 大 UI：按行做集合 diff，未变化只统计数量
    """
    before_lines = _ui_to_lines(ui_before)
    after_lines = _ui_to_lines(ui_after)

    if len(before_lines) <= _FULL_DUMP_LINE_THRESHOLD and len(after_lines) <= _FULL_DUMP_LINE_THRESHOLD:
        # 小 UI 直接附原文，让模型自己对比
        full = (
            "操作前界面：\n"
            f"{ui_before if before_lines else '(空)'}\n\n"
            "操作后界面：\n"
            f"{ui_after if after_lines else '(空)'}"
        )
        return full, before_lines != after_lines

    # 大 UI → 行级集合 diff（保持顺序去重）
    ...
    unchanged = len(before_lines) - len(removed)

    has_changes = bool(removed) or bool(added)
    if not has_changes:
        return (
            f"界面完全无变化（操作前后共 {len(before_lines)} 行界面元素，逐行一致）。",
            False,
        )

    def _truncate(lines: list[str], label: str) -> list[str]:
        if len(lines) <= _DIFF_MAX_LINES_PER_BUCKET:
            return lines
        keep = lines[:_DIFF_MAX_LINES_PER_BUCKET]
        return keep + [f"…（{label}还有 {len(lines) - _DIFF_MAX_LINES_PER_BUCKET} 行未显示）"]

    parts = [
        f"界面变化摘要（差异对比模式，节省 token）：",
        f"  - 未变化的元素：{unchanged} 行（已省略）",
        f"  - 消失（操作前有、操作后无）：{len(removed)} 行",
    ]
    for ln in _truncate(removed, "消失"):
        parts.append(f"    [-] {ln}")
    parts.append(f"  - 新增（操作前无、操作后有）：{len(added)} 行")
    for ln in _truncate(added, "新增"):
        parts.append(f"    [+] {ln}")
    return "\n".join(parts), True
...
class Reflector:
    ...
    def verify(self, current_step: str, action_taken: dict,
               ui_before: str, ui_after: str,
               experiences: str = "") -> tuple[dict, TokenUsage]:

        action_type = action_taken.get("action", "")
        # 用行级集合判断变化，比 strip() 更可靠：忽略空行/末尾空白，
        # 但仍能精确识别"任何元素变化"
        before_lines = _ui_to_lines(ui_before)
        after_lines = _ui_to_lines(ui_after)
        ui_changed = set(before_lines) != set(after_lines)
        ...
        # ── 关键优化：UI 较大时只传 diff，节省 50%+ token ─────────────
        ui_block, _ = _format_ui_diff(ui_before, ui_after)

        experience_block = f"\n{experiences}\n" if experiences else ""
        prompt = f"""当前里程碑步骤：{current_step}
执行的操作：{json.dumps(action_taken, ensure_ascii=False)}
界面是否发生变化：{"是" if ui_changed else "否"}{no_change_warning}{experience_block}
{ui_block}
...
请判断该操作对当前里程碑步骤的推进效果（done/progress/stuck）。"""
        ...
        try:
            ...
            # 强制 status 字段存在；移除原"兼容 success: true/false"死代码（系统内部已无此格式）
            ...
            print(f"[Reflector] JSON 解析失败（{e}），raw 前 300 字：{rsp[:300]!r}")
